<template>
  <div v-if="current">
    <quick-view-panel :open="showCreatePanel" :heading="$t('Add variant option')" @close="showCreatePanel = false">
      <create-variant :options="blog.option_data || {}" :blog-id="blog.id" :errors="createErrors" @saved="handleOptionCreate" />
    </quick-view-panel>
    <simple-modal
    ...
    <div class="flex">
      ...
      <div class="w-full bg-white">
        <template v-if="blogVariantCount > 1">
          ...
          <div class="p-6">
            <gc-asset-picker :assets="blogImages" :current="current.image" @save="handleAssetChange">
              <div slot="empty">
                <div class="p-3 mb-2 text-sm text-blue-700 bg-blue-100 rounded">
                  {{ $t('If no image is specified you can still use the default image for the blog') }}
                </div>
              </div>
            </gc-asset-picker>
          </div>
        ...
        <div>
          ...
          <div class="p-6">
            <div class="md:grid md:grid-cols-3 md:gap-6">
              ...
              <form-field :label="$t('Min Batch Quantity')" :instructions="$t('This blog can only be ordered in multiples of {min_batch}', { min_batch: current.min_batch })">
...
export default {
  ...
  computed: {
    blogImages () {
      return filter(get(this.blog, 'assets.data', []), asset => asset.kind === 'image')
    },
    hasOptionData () {
      return !!Object.keys(this.blog.option_data).length
    },
    blogVariantCount () {
      return this.blogVariants.length
    },
    blogVariants () {
      return this.blog.variants.data
    },
    blog () {
  ...
  mounted () {
    ...
    each(this.blog.option_data, (option, handle) => {
  ...
  methods: {
    ...
    syncVariants () {
      this.variants = JSON.parse(
        JSON.stringify(this.blogVariants)
      )
    },
    selectVariant (index) {
    ...
    async handleAssetChange (assetId) {
      ...
      const asset = find(this.blogImages, asset => asset.id === assetId)
    ...
    async handleOptionCreate (val) {
      ...
      try {
        const response = await this.$gc.blogVariants.create(this.blog.id, {
          variants: [val]
        })
