    ...
    <div v-else>
      <div>
        <header class="mb-2 font-bold text-gray-700 text-sm">
          <label>{{ $t('Blog family') }}</label>
          <p class="text-red-600 leading-tight text-xs font-normal">
            {{ $t('Changing blog family may lead to loss of data') }}
          </p>
        </header>
        <div>
          <auto-complete
...
export default {
  ...
  methods: {
    async searchFamilies (keywords) {
      const response = await this.$gc.blogFamilies.get({
        keywords
      })

      this.families = response.data.data
    },
    setBlogFamily (family) {
